	...
	$(function(){
		...
		$("#deleteBtn").click(function () {
			...
			if(confirm("您确定要删除吗？")){
				if($xz.length==0){
					...
					$.post("customer/doDelete",param,function (data) {
						if(data){
							alert("删除成功");
							pageList();
						}else{
		...
		$("#editBtn").click(function () {
			...
			if($xz.length==0){
				...
				$.get("customer/getUserListAndCustomer/"+id+"",null,function (data) {
					var html = "";
			...
			success:function (data) {
				...
				if(JSON.stringify(data.list) == "[]"){
					...
					$.each(data.list,function (i,n) {
						...
						html += '<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'customer/toDetail/'+n.id+'\';">'+n.name+'</a></td>';
						html += '<td>'+n.owner+'</td>';
						html += '<td>'+n.phone+'</td>';
